...
# E2E NETWORKS
LID_SERVICE_URL = os.environ.get("LID_SERVICE_URL", "http://165.232.178.107:6001")
INDIC_EN_SERVICE_URL = "http://164.52.194.212:6002"
EN_INDIC_SERVICE_URL = "http://164.52.194.212:6003"
...
def _clean_translation_output(text: str) -> str:
    """
    Strip leading dots and spaces that IndicTrans2 models sometimes prepend.
    
    The model frequently generates a leading ". " or ".. " prefix in its output.
    Python's str.lstrip(". ") treats the argument as a *character set* (stripping
    any mix of '.', ' '), which works but can over-strip in edge cases.
    re.sub with an anchored pattern is safer and explicit.
    """
    if not text:
        return text
...
def simplify_response(full_response: dict) -> dict:
    ...
    for trans in full_response.get("translations", []):
        output = trans.get("final", "") or trans.get("translated", "") or trans.get("output", "")
        output = _clean_translation_output(output)
        outputs.append(output)
...
async def translate(req: TranslateRequest):
    ...
    if tgt_lang == "eng_Latn":
        ...
        # Same language — but if input is Roman, we still need transliteration.
        # Route through LID check which handles both cases:
        #   Roman input  → returns transliterated (native script) text
        #   Native input → returns input as-is
        logger.info(f"[{request_id}] src_lang == tgt_lang ({src_lang}), checking if transliteration needed")
        full_result = await _translate_with_lid_check(request_id, sentences, src_lang, tgt_lang, False, req, t_start)
...
async def translate_detailed(req: TranslateRequest):
    ...
    if tgt_lang == "eng_Latn":
        ...
        # Same language — but if input is Roman, we still need transliteration.
        logger.info(f"[{request_id}] src_lang == tgt_lang ({src_lang}), checking if transliteration needed")
        return await _translate_with_lid_check(request_id, sentences, src_lang, tgt_lang, False, req, t_start)
...
async def _translate_indic_to_indic(
    ...
    for t in step1_result.get("translations", []):
        en_text = t.get("final", "") or t.get("translated", "") or ""
        en_text = _clean_translation_output(en_text)
        en_sentences.append(en_text)
